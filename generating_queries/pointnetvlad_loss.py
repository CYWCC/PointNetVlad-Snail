...
def triplet_loss_sim(anchor, positive, negative, margin, use_min=False, lazy=False):
    min_pos, max_pos = best_pos_distance(anchor, positive)
    if use_min:
        positive = min_pos
    else:
        positive = max_pos
    ...
    query_copies = anchor.repeat(1, int(num_neg), 1)
    positive = positive.view(-1, 1)
    positive = positive.repeat(1, int(num_neg))

    loss = margin + positive - ((negative - query_copies) ** 2).sum(2)
    loss = loss.clamp(min=0.0)
    if lazy:
        triplet_loss = loss.max(1)[0]
    else:
...
def similar_quadruple_loss(q_vec, pos_vecs1, pos_vecs2, neg_vecs, m1, m2, use_min=False, lazy=False):
    loss1 = triplet_loss_sim(q_vec, pos_vecs1, neg_vecs, m1, use_min, lazy)
    loss2 = triplet_loss_sim(q_vec, pos_vecs2, neg_vecs, m1, use_min, lazy)
    loss3 = triplet_loss_sim(q_vec, pos_vecs1, pos_vecs2, m2, use_min, lazy)

    total_loss = loss1 + loss2 + loss3
    return total_loss

def Latest_farthest_loss(q_vec, pos_vecs1, pos_vecs2, neg_vecs, other_neg, m1, m2, m3, use_min=False, lazy=False, ignore_zero_loss=False):
    loss1 = quadruplet_loss(q_vec, pos_vecs1, neg_vecs, other_neg, m1, m2, use_min, lazy, ignore_zero_loss)
    loss2 = quadruplet_loss(q_vec, pos_vecs2, neg_vecs, other_neg, m1, m2, use_min, lazy, ignore_zero_loss)
    loss3 = triplet_loss_sim(q_vec, pos_vecs1, pos_vecs2, m3, use_min, lazy)
